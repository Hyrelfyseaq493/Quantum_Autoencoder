...
import os
import json
from model_qa import CircuitSearchModel, NAS_search_space, circuit_search, Rs_space, CNOTs_space
...
parser.add_argument('--test_result_save', type=str, default='testing_result_with_auc', help='experiment name')
parser.add_argument('--model_save', type=str, default='model', help='experiment name')
...
def cost_fn(params, model, subnet, inputs, labels, records):
    ...
    records['loss'].append(float(loss._value))
    return loss

def cost_fn(params, model, subnet, inputs, records):
    loss = 0
    correct = 0
    size = inputs.shape[0]
    model.params = params
...
def expert_evaluator(model, subnet, inputs, labels, n_experts):
    ...
    for i in range(n_experts):
        ...
        if target_loss == 0 or temp_loss < target_loss:
            target_loss = temp_loss
            target_expert = i
    return target_expert

def test_expert_evaluator(model, subnet, inputs, labels, n_experts):
    r''' In this function, we locate the expert that achieves the minimum loss, where such an expert is the best choice
     for the given subset'''
    target_expert = 0
    target_loss = 0
    for i in range(n_experts):
        ...
        for data, label in zip(inputs, labels):
            ...
            if label == 1:
                ...
                print("error")
        bg_loss = 2*bg_loss/len(labels)
        sg_loss = 2*sg_loss/len(labels)
        if bg_loss < sg_loss:# and bg_loss < 0.1 and sg_loss>0.9:
            target_loss = sg_loss - bg_loss
            target_expert = i
    return target_expert
...
def main():
    ...
    if args.noise:
        ...
        dev = qml.device("default.qubit", wires=latent_size+2*trash_size+1)
    ...
    sg1_data = data[:500][1]
    sg2_data = data[:500][2]
    bg_label = data[:2500][3]
    sg1_label = data[:500][4]
    sg2_label = data[:500][5]
    data_train = bg_data[:1000]
    label_train = bg_label[:1000]
    data_valid = bg_data[1000:2000]
    label_valid = bg_label[1000:2000]
    data_test1 = np.vstack((bg_data[2000:], sg1_data))
    label_test1 = np.concatenate((bg_label[2000:], sg1_label)).flatten()
    data_test2 = np.vstack((bg_data[2000:], sg2_data))
    label_test2 = np.concatenate((bg_label[2000:], sg2_label)).flatten()
    ...
    for epoch in range(args.epochs):
        subnet = np.random.randint(0, len(Rs_space), (args.n_layers,)).tolist()
        subnet.append(np.random.randint(0, len(CNOTs_space), 1).tolist()[0])
        #subnet = np.random.randint(0, len(NAS_search_space), 1).tolist()
        ...
        print('{}/{}: subnet: {}, expert_idx: {}'.format(epoch+1,args.epochs,subnet, expert_idx))
        model.params_space[expert_idx] = opt.step(lambda params: cost_fn(params, model, subnet, data_train, records), model.params_space[expert_idx]) # train
#        with open(f"circuit_diagram.txt", "w") as f:
#            fig, ax = qml.draw_mpl(
#                model(data_train,subnet), expansion_strategy="device"#, max_length=1000
#                , style="pennylane"
 #           )(data_train,0)
 #       fig.savefig(f"/Users/hyrelfyseaq493/Downloads/Quantum_architecture_search-main/machine_learning/circuit_plot/{subnet}")
    #model.save(args.model_save)

    '''search'''
    print('Start search.')
    result = {}
    test_result = np.empty([args.n_search,6])
    if args.searcher == 'random':
        for i in range(args.n_search):
            subnet = np.random.randint(0, len(Rs_space), (args.n_layers,)).tolist()
            subnet.append(np.random.randint(0, len(CNOTs_space), 1).tolist()[0])
            expert_idx1 = test_expert_evaluator(model, subnet, data_test1, label_test1, args.n_experts)
            auc_1 = tpr_fpr(model, subnet, expert_idx1, data_test1, label_test1)
            expert_idx2 = test_expert_evaluator(model, subnet, data_test2, label_test2, args.n_experts)
            auc_2 = tpr_fpr(model, subnet, expert_idx2, data_test2, label_test2)
#            result['-'.join([str(x) for x in subnet])] = bg_loss, sg_loss 
            print('{}/{}: subnet: {}, auc of sg1: {}, auc of sg2: {}'.format(i+1, args.n_search, subnet, auc_1, auc_2))
            test_result[i,:-2] = subnet
            test_result[i,-2] = auc_1
            test_result[i,-1] = auc_2
